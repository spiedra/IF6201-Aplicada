...
CREATE PROCEDURE OMAZON.sp_AUTENTICACACION 
	@param_NOMBRE_USUARIO VARCHAR(50),
	@param_CONTRASENIA VARCHAR(50)
AS
BEGIN
	IF NOT EXISTS (SELECT [NOMBRE_USUARIO], [CONTRASENNA] FROM [OMAZON].[USUARIO] WHERE NOMBRE_USUARIO = @param_NOMBRE_USUARIO
																												AND CONTRASENNA = @param_CONTRASENIA)
		BEGIN
	...
	ELSE
		BEGIN
			SELECT 
				U.[ID_ROLE] AS RESPONSE,
				R.TIPO_ROLE
			FROM [OMAZON].[USUARIO] U
				JOIN [OMAZON].[ROLE] R
					ON U.ID_ROLE = R.ID_ROLE
			WHERE U.NOMBRE_USUARIO = @param_NOMBRE_USUARIO AND U.CONTRASENNA = @param_CONTRASENIA
		END
END

ALTER PROCEDURE OMAZON.sp_REGISTRAR_COMPRA_BY_CARRITO
	@param_ID_CLIENTE INT
AS
BEGIN
	DECLARE @local_ID_VENTA INT
	DECLARE @local_SUB_TOTAL INT
	DECLARE @local_COUNT INT
	DECLARE @local_TOTAL_VENTA INT
	DECLARE @local_TOTAL_PRODUCTOS INT = 
									(SELECT 
										 count(ID_CLIENTE) 
											 FROM [IF6201_OMAZON_P_APLICADA2021].[OMAZON].[CARRITO_COMPRAS]
												WHERE ID_CLIENTE = @param_ID_CLIENTE)
	SET @local_COUNT = 0
	SET @local_TOTAL_VENTA = 0

	INSERT INTO [OMAZON].[VENTA]([FECHA_VENTA])
	VALUES(GETDATE())
    SET @local_ID_VENTA = SCOPE_IDENTITY()

	WHILE @local_COUNT < @local_TOTAL_PRODUCTOS
		BEGIN
			DECLARE @local_ID_COMPRA INT
			DECLARE @local_ID_CARRITO_COMPRAS INT = (SELECT TOP (1) ID_CARRITO_COMPRAS 
												FROM [OMAZON].[CARRITO_COMPRAS]
													WHERE ID_CLIENTE = @param_ID_CLIENTE)
			DECLARE @local_ID_PRODUCTO INT = (SELECT ID_PRODUCTO 
												FROM [OMAZON].[CARRITO_COMPRAS]
													WHERE ID_CARRITO_COMPRAS = @local_ID_CARRITO_COMPRAS)
			SET @local_SUB_TOTAL = (SELECT TOP (1) SUB_TOTAL 
												FROM [OMAZON].[CARRITO_COMPRAS]
													WHERE ID_CARRITO_COMPRAS = @local_ID_CARRITO_COMPRAS)
									
			INSERT INTO [OMAZON].[COMPRA]([ID_CLIENTE], [ID_PRODUCTO], [TOTAL], [ID_VENTA])
			VALUES(@param_ID_CLIENTE, @local_ID_PRODUCTO, @local_SUB_TOTAL, @local_ID_VENTA)
			DELETE FROM [OMAZON].[CARRITO_COMPRAS] WHERE ID_CARRITO_COMPRAS = @local_ID_CARRITO_COMPRAS

			SET @local_TOTAL_VENTA = @local_SUB_TOTAL + @local_TOTAL_VENTA

			SET @local_COUNT = @local_COUNT + 1;
		END
	UPDATE [OMAZON].VENTA
		SET MONTO_TOTAL = @local_TOTAL_VENTA
	WHERE ID_VENTA = @local_ID_VENTA
END

DELETE FROM OMAZON.COMPRA

DELETE FROM OMAZON.VENTA